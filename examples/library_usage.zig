...
fn runBuiltinProcessorsExample(allocator: std.mem.Allocator, listenIP: []const u8, listenPort: u16, forwardIP: []const u8, forwardPort: u16) !void {
    ...
    try stdout.print("\n1. Pass-Through Processor:\n", .{});
    {
        ...
        var proxy = try udp_proxy.ProxySocketPairGeneric(udp_proxy.PassThroughProcessor).init(
            processor, listenIP, listenPort, forwardIP, forwardPort
        );
        defer proxy.deinit();
    ...
    try stdout.print("\n2. Logging Processor:\n", .{});
    {
        ...
        var proxy = try udp_proxy.ProxySocketPairGeneric(udp_proxy.LoggingProcessor).init(
            processor, listenIP, listenPort, forwardIP, forwardPort
        );
        defer proxy.deinit();
    ...
    try stdout.print("\n3. Filtering Processor:\n", .{});
    {
        ...
        var proxy = try udp_proxy.ProxySocketPairGeneric(udp_proxy.FilteringProcessor).init(
            processor, listenIP, listenPort, forwardIP, forwardPort
        );
        defer proxy.deinit();
    ...
    try stdout.print("\n4. Custom Processor:\n", .{});
    {
        ...
        var proxy = try udp_proxy.ProxySocketPairGeneric(udp_proxy.CustomProcessor).init(
            processor, listenIP, listenPort, forwardIP, forwardPort
        );
        defer proxy.deinit();
    ...
    try stdout.print("\n5. SRT NAK Malformer:\n", .{});
    {
        ...
        var proxy = try udp_proxy.ProxySocketPairGeneric(udp_proxy.SRTNakMalformerProcessor).init(
            processor, listenIP, listenPort, forwardIP, forwardPort
        );
        defer proxy.deinit();
    ...
    try stdout.print("\n6. Packet Dropper (drops packets 99, 100, 101):\n", .{});
    {
        ...
        var proxy = try udp_proxy.ProxySocketPairGeneric(DropperType).init(
            processor, listenIP, listenPort, forwardIP, forwardPort
        );
        defer proxy.deinit();
