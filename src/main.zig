...
pub fn main() !void {
    ...
    const processor = try processors.selectProcessor(allocator, processor_type, args);

    // Compact proxy initialization
    var proxy_instance = proxy.ProxySocketPairGeneric(@TypeOf(processor))
        .init(processor, listenIP, listenPort, forwardIP, forwardPort) catch |err| {
