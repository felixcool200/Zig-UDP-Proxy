...
pub const Processor = union(enum) {
    ...
    pub fn processListenerToForward(self: *@This(), buffer: []u8, packet: []u8) []u8 {
        return switch (self.*) {
            .PassThrough => |*p| p.processListenerToForward(buffer, packet),
            .Logging => |*p| p.processListenerToForward(buffer, packet),
            .Filtering => |*p| p.processListenerToForward(buffer, packet),
            .Custom => |*p| p.processListenerToForward(buffer, packet),
            .SRTNak => |*p| p.processListenerToForward(buffer, packet),
            .SRTNakBidir => |*p| p.processListenerToForward(buffer, packet),
            .Dropper => |*p| p.processListenerToForward(buffer, packet),
        };
    }

    pub fn processForwardToListener(self: *@This(), buffer: []u8, packet: []u8) []u8 {
        return switch (self.*) {
            .PassThrough => |*p| p.processForwardToListener(buffer, packet),
            .Logging => |*p| p.processForwardToListener(buffer, packet),
            .Filtering => |*p| p.processForwardToListener(buffer, packet),
            .Custom => |*p| p.processForwardToListener(buffer, packet),
            .SRTNak => |*p| p.processForwardToListener(buffer, packet),
            .SRTNakBidir => |*p| p.processForwardToListener(buffer, packet),
            .Dropper => |*p| p.processForwardToListener(buffer, packet),
        };
    }
};
